...
export const LanguageSelectorDesktop = ({ localeName, onChange, displayName }) => {
  const currentLocale = useCurrentLocale({ defaultLocale, locales});
    ...
    <div className="relative" ref={containerRef}>
      ...
      <FocusLock disabled={!isOpen} returnFocus={true}>
        <ul
          ref={menuRef}
          className={twMerge(
            'top-100 absolute right-0 z-10 w-24 translate-y-3 cursor-pointer rounded-md bg-colorWhite text-center text-base shadow-sm',
            isOpen ? 'block' : 'hidden',
          )}
          id="menu-locale"
          role="menu"
